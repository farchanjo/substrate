...
//!
//! Delivers a POSIX signal to a target process. Enforces:
//!   1. Dry-run gate: first-pass returns preview, no OS mutation.
//!   2. Elicitation gate (ADR-0004 Layer 4): SIGKILL/SIGTERM/SIGSTOP require
//!      `elicitation_confirmed = true` before delivery.
//!   3. PID existence check via `kill(pid, 0)`.
//!
...
/// # Errors
///
/// - `SubstrateError::NotFound` when the target PID does not exist.
...
/// - `SubstrateError::PermissionDenied` when the OS rejects the signal.
...
pub async fn handle_proc_signal(
    ...
    // Layer 2: Dry-run gate (ADR-0004 Layer 3).
    // Default is dry-run mode; only proceed if explicitly false.
    ...
    // Layer 3: Elicitation gate for destructive signals (ADR-0004 Layer 4).
