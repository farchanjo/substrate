//! Shared response types and dependency bundle for all system-info tool handlers.
//!
//! `SystemInfoDeps` is the single struct threaded through every handler so the
...
/// Dependency bundle for all system-info tool handlers.
///
/// The composition root constructs this once and shares it across concurrent
/// handler invocations via `Arc<SystemInfoDeps>`.
#[derive(Clone)]
pub struct SystemInfoDeps {
    /// Runtime capability snapshot — used to annotate capability tier in hints.
    pub capabilities: Arc<Capabilities>,
}

impl std::fmt::Debug for SystemInfoDeps {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SystemInfoDeps")
            .field("capabilities", &self.capabilities)
            .finish_non_exhaustive()
    }
}

/// The response envelope returned by every system-info handler.
///
