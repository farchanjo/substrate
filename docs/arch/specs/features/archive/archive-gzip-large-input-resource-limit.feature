Feature: archive.gzip.compress enforces resource limit for large inputs
  ...
  Scenario: Input > 1 GiB without allow_large=true returns SUBSTRATE_RESOURCE_LIMIT
    ...
    When the client calls archive.gzip.compress with src="/work/repo/data/huge.bin" and allow_large=false
  ...
  Scenario: Input > 1 GiB with allow_large=true proceeds
    Given the file "/work/repo/data/huge.bin" has a size of 1.5 GiB
    When the client calls archive.gzip.compress with src="/work/repo/data/huge.bin" and allow_large=true and elicitation_confirmed=true
  ...
  Scenario: Input exactly at 1 GiB boundary without allow_large returns SUBSTRATE_RESOURCE_LIMIT
    ...
    When the client calls archive.gzip.compress with src="/work/repo/data/boundary.bin" and allow_large=false
    Then the tool returns error code SUBSTRATE_RESOURCE_LIMIT

  Scenario: Input below 1 GiB does not require allow_large
    Given the file "/work/repo/data/small.bin" has a size of 512 MiB
    When the client calls archive.gzip.compress with src="/work/repo/data/small.bin" and allow_large=false and elicitation_confirmed=true
