Feature: archive.tar.create supports dry-run plan and confirmed archive creation
  ...
  Scenario: Dry run returns plan without writing to disk
    When the client calls archive.tar.create with src="/work/repo/src" and dst="/work/repo/dist/src.tar.gz" and dry_run=true
  ...
  Scenario: Confirmed creation after elicitation writes the archive
    ...
    When the client calls archive.tar.create with src="/work/repo/src" and dst="/work/repo/dist/src.tar.gz" and dry_run=false and elicitation_confirmed=true
  ...
  Scenario: Creation without elicitation confirmation returns SUBSTRATE_DRY_RUN_REQUIRED
    When the client calls archive.tar.create with src="/work/repo/src" and dst="/work/repo/dist/src.tar.gz" and dry_run=false and elicitation_confirmed=false
