...
// Entries are grouped into the nested #*ErrorEntries fragments below, one per ADR
// family; each fragment is scoped to this definition and embedded in place, so the
// catalog stays a single flat map.
#ErrorCatalog: {
	// DDD role: ValueObject
	// #PathSecurityErrorEntries covers the allowlist and path-jail violations.
	#PathSecurityErrorEntries: {
	...
	// DDD role: ValueObject
	// #CoreErrorEntries covers the generic lifecycle and resource outcomes.
	#CoreErrorEntries: {
	...
	// DDD role: ValueObject
	// #ContractErrorEntries covers argument, consent, and protocol-contract failures.
	#ContractErrorEntries: {
	...
	// DDD role: ValueObject
	// #KernelErrorEntries covers the kernel-induced I/O errors.
	#KernelErrorEntries: {
	...
	// DDD role: ValueObject
	// #StartupErrorEntries covers the startup contract errors per ADR-0036.
	#StartupErrorEntries: {
	...
	// DDD role: ValueObject
	// #JobErrorEntries covers the async-job BC errors per ADR-0040.
	#JobErrorEntries: {
	...
	// DDD role: ValueObject
	// #CapabilityErrorEntries covers the capability-startup errors per ADR-0042 + ADR-0043.
	#CapabilityErrorEntries: {
	...
	// DDD role: ValueObject
	// #SubprocessSecurityErrorEntries covers the subprocess admission violations per ADR-0052.
	#SubprocessSecurityErrorEntries: {
	...
	// DDD role: ValueObject
	// #SubprocessLifecycleErrorEntries covers the subprocess lifecycle and streaming errors per ADR-0052 + ADR-0053 + ADR-0054 + ADR-0055.
	#SubprocessLifecycleErrorEntries: {
	...
	// DDD role: ValueObject
	// #LaunchTrustErrorEntries covers the launch profile trust and config-hygiene errors per ADR-0064.
	#LaunchTrustErrorEntries: {
	...
	// DDD role: ValueObject
	// #LaunchLifecycleErrorEntries covers the launch dependency and orphan-governance errors per ADR-0065 + ADR-0068.
	#LaunchLifecycleErrorEntries: {
	...
	// DDD role: ValueObject
	// #LaunchHardeningErrorEntries covers the launch supervisor-hardening errors per ADR-0068.
	#LaunchHardeningErrorEntries: {
