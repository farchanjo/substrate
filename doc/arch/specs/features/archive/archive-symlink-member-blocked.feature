Feature: archive.zip.extract blocks symlink members that point outside the extraction root
  ...
  Scenario: Archive whose first member is a symlink pointing outside extraction root is rejected before any write
    ...
    When the client calls archive.zip.extract with archive="/work/repo/symlink_escape.zip" and dst="/work/repo/extracted/"
  ...
  Scenario: Symlink entry pointing to an absolute path outside extraction root is rejected
    ...
    When the client calls archive.zip.extract with archive="/work/repo/abs_symlink.zip" and dst="/work/repo/extracted/"
    Then the tool returns error code SUBSTRATE_PATH_TRAVERSAL_BLOCKED
  ...
  Scenario: Symlink entry pointing inside the extraction root is allowed
    ...
    When the client calls archive.zip.extract with archive="/work/repo/safe_symlink.zip" and dst="/work/repo/extracted/"
  ...
  Scenario: Validation happens before any file write — no partial extraction on rejection
    ...
    When the client calls archive.zip.extract with archive="/work/repo/mixed.zip" and dst="/work/repo/extracted/"
    Then the tool returns error code SUBSTRATE_PATH_TRAVERSAL_BLOCKED
    And no files are written to disk in "/work/repo/extracted/"
  ...
  Scenario: Symlink loop created via archive member is blocked
    ...
    When the client calls archive.zip.extract with archive="/work/repo/loop.zip" and dst="/work/repo/extracted/"
    Then the tool returns error code SUBSTRATE_PATH_TRAVERSAL_BLOCKED
