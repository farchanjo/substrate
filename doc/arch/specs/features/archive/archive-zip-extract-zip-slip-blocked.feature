Feature: archive.zip.extract blocks zip-slip path traversal before any write
  ...
  Scenario: Archive with traversal member "../evil.txt" is rejected before any write
    ...
    When the client calls archive.zip.extract with archive="/work/repo/evil.zip" and dst="/work/repo/extracted/"
  ...
  Scenario: Archive with nested traversal "a/../../outside.txt" is rejected
    ...
    When the client calls archive.zip.extract with archive="/work/repo/nested_slip.zip" and dst="/work/repo/extracted/"
    Then the tool returns error code SUBSTRATE_PATH_TRAVERSAL_BLOCKED
    And no files are written to disk in "/work/repo/extracted/"
  ...
  Scenario: Legitimate archive with all members inside target extracts successfully
    ...
    When the client calls archive.zip.extract with archive="/work/repo/safe.zip" and dst="/work/repo/extracted/"
