...
behavioral and architectural change starts in `doc/arch/` (ADR + CUE + Gherkin
...
under `doc/arch/adr/` (see [ADR-0001](doc/arch/adr/0001-record-architecture-decisions.md))
before writing code.
...
Branch naming (per [ADR-0024](doc/arch/adr/0024-repo-conventions.md)):
...
Every change under `doc/arch/` must pass:
...
See [doc/arch/README.md](doc/arch/README.md) for the spec layout and
individual linters.

### 4. Build, lint, test

Before running these, run `spec validate --lane fast` (see section 3 above) if you
touched anything under `doc/arch/`.
...
Use the Angular convention from [ADR-0024](doc/arch/adr/0024-repo-conventions.md):
...
- **Stack**: Rust 1.95 (edition 2024), rmcp 1.7 (`server`/`transport-io`/`macros`
  ...
  See [ADR-0003](doc/arch/adr/0003-crate-stack-and-async-zones.md), [ADR-0006](doc/arch/adr/0006-tokio-runtime-timeout-cancellation.md).
- **Transport**: STDIO only. `stdout` is sacred. `println!`/`print!` forbidden
  in `src/`. See [ADR-0005](doc/arch/adr/0005-stdio-transport.md).
- **Security**: allowlist → path jail (`openat2`/`O_NOFOLLOW_ANY`) → dry-run →
  elicitation. See [ADR-0004](doc/arch/adr/0004-security-model.md) and
...
- **Subprocess**: an opt-in bounded context gated behind the Cargo feature
  ...
  ([ADR-0044](doc/arch/adr/0044-no-subprocess-policy.md)) is superseded by
  [ADR-0052](doc/arch/adr/0052-subprocess-execution-architecture.md).
- **Signal safety**: `SIGPIPE` ignored at startup; `SIGTERM`/`SIGINT` drain.
  See [ADR-0032](doc/arch/adr/0032-signal-safety.md).
- **Cancellation**: `tokio-util` `CancellationToken` + `tokio::select! biased`.
  See [ADR-0037](doc/arch/adr/0037-async-cancellation-patterns.md).
- **Error taxonomy**: `SUBSTRATE_<UPPER_SNAKE>` codes; every error carries
  `code`, `message_en_us`, `recovery_hint`, `correlation_id`. See
  [ADR-0010](doc/arch/adr/0010-error-taxonomy.md).

## Adding a new tool

1. Draft an ADR if the tool introduces new semantics or crosses a bounded
   context.
2. Add a Gherkin feature under `doc/arch/specs/features/<bc>/` capturing the
   happy path + at least one failure mode.
3. Add a CUE schema under `doc/arch/schemas/` if the tool exposes a new
   value object.
